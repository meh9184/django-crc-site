...
$("button[name=addRow]").click(function () {
	...
	$("input[name=newratio]").prop("name", oldname+"r");


    $("#tagtbody").prepend(rowTemplate.clone());
...
$("button[name=saveServer]").click(function () {
    ...
    $("#wordpartTable tbody tr").each(function () {
        ...
        sendingRow = new SendingRow(
			...
			$(':radio[class="ratioflag"]:checked', this).val()
        );
        ...
        $.ajax({
            ...
            success: function (resp) {
               console.log(resp);
				k++;
				alert("✔ "+k+"번째 태그: "+ resp);
				if (resp == "success - 저장 성공하였습니다."){
					i++;
				}else{
					j++;
				}
	     	  	$("#staMsg").text(i+"개저장 성공, "+j+"개저장 실패. 저장하신 리스트를 최신화 하시려면  F5(새로고침)을 눌러주세요!");
            }

        });
	} else {
		$("#staMsg").text("저장 실패:   필수 요소를 선택 또는 입력해주세요!");
		alert("저장 실패:  필수 요소를 선택 또는 입력해주세요!");
		}
...
function SendingRow(sub, obj, verb, st, etc, subflag, obflag, vbflag, etflag, passflag, raflag) {
    var sendingRow = {
        aid: $('#aid').val(),
        subject: sub,
        object: obj,
